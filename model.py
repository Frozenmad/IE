...
class SequenceLabelingModel(object):
    ...
    def build_model(self):
        ...
        for feature_name in self.feature_names:
            ...
            # init dropout rate, 如果没有指定某一项的Dropout rate，那么初始化该项为0
        ...
        for feature_name in self.feature_names:
            #tf.nn.embedding_lookup 这个函数用来返回param中第ids行组成的元素，返回值为tensor
            #tf.nn.dropout 这个函数按照keep_prob参数来舍弃它param中的元素，返回值为tensor
    ...
    def split_train_dev(self, data_dict, dev_size=0.2):
        ...
        # get a copy and transfer all the data to numpy
        for _,elem in enumerate(data_dict_copy):
            data_dict_copy[elem] = np.array(data_dict_copy[elem])
            lenth = len(data_dict_copy[elem])
            break
        # get a shuffled list
        shuffle_list = np.random.permutation(lenth)
        # shuffle the data
        for _,elem in enumerate(data_dict_copy):
            data_dict_copy[elem] = data_dict_copy[elem][shuffle_list]

        for name in data_dict_copy.keys():
            boundary = int((1.-dev_size) * data_dict_copy[name].shape[0])
            data_train_dict[name] = data_dict_copy[name][:boundary]
            data_dev_dict[name] = data_dict_copy[name][boundary:]
